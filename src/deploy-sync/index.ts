...
class Hashable {
    ...
    private generateHashStr(): void {
        ...
        for (const key of this.hashKeys) {
            let v = eval(`this.${key}`);
            if (this.config.checkUndefAndNull ? (v !== undefined && v !== null) : true) { // Here we could check about the validity of v (not undefined, not null, or we can include those in the hash.
                if (v === undefined) {
                    v = 'undefined';
                }
                if (v === null) {
...
class Deployer {
    ...
    constructor(id: string, source?: DeploySource, user?: DeployUser, options?: DeployOptions, server?: DeployServer, destination?: DeployDestination) {
        this.id = id;
        this.source = source;
        this.user = user;
...
class DeployOptions<T = Rsync> {
    ...
    public configure(executor: CommandShell): void {
        ...
        Object.entries(this.config as any).filter(([k, v]) => (this.config as any).hasOwnProperty(k) && typeof executor[k] === 'function').map(([k, v]) => {
    ...
    getGeneratorString(executor: any): string {
        ...
        str = Object.entries(this.config as any).filter(([k, v]) => (this.config as any).hasOwnProperty(k) && executor[k]).map(([k, v]) => {
            if (typeof this.config[k] === 'boolean') {
                return `${k}()`;
            }
            if (Array.isArray(this.config[k])) {
...
export class DeploySync {
    ...
    public static newDeployer(name: string | Object): Deployer {
        ...
        if (typeof name === 'string') {
            ...
            deployer = new Deployer(config.id, DeploySync.getSource(config.source), DeploySync.getUser(config.user), DeploySync.getOptions(config.options), DeploySync.getServer(config.server), DeploySync.getDestination(config.dest));
    ...
    public static newDeployment(id: string, fromDeployerOrConfig?: Deployer | string, overwrite: boolean = false, attachedResults?: ExecutionResults): Deployment {
