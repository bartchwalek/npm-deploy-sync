...
export class Rsync extends CommandShell {
    ...
    constructor(configuration: RsyncConfig) {
        super(
            ...
            configuration.useSshPass || configuration.useGPG ? `$special $flags $options $map[local] $map[dest]` : '$flags $options $map[local] $map[dest]',
            () => {
    ...
    public setClearTextPassword(password: string): void {
        this.addSpecial(['-p', `${password}`, 'rsync']);
    }

    public setGPG(file: string): void {
