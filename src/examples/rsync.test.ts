...
rs.getFormattedResults('FirstCopy').then((results: RsyncFormattedResults) => {
    ...
    console.log(`${results.formatted.files.sent} ${results.formatted.files.sentType} were sent at ${results.formatted.files.speed} ${results.formatted.files.speedType}`);
